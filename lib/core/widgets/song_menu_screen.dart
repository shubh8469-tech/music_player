...
class _SongMenuScreenState extends State<SongMenuScreen> {
  ...
  Widget build(BuildContext context) {
    // Determine divider positions based on context
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        // Cap to ~85% of available height so it never goes full-screen
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (widget.from == 'playlist' && widget.playlist != null)
                      Padding(
                        ...
                        child: MusicListTile(
                          ...
                          cardIconSize: 32.r,
                          isSvgCardIcon: (widget.playlistIconAsset ?? '')
                              .contains('.svg'),
                    ...
                    // Menu items (your existing map)
                    ...widget.songMenuList.map((songItem) {
                            ...
                            onTap: () async {
                              ...
                              if (songItem.title == localization.keepScreenOn) {
                                ...
                                if (widget.from == 'playlist') {
                                  ...
                                  try {
                                    ...
                                    bloc.add(SetPlaylistEvent(
                                      _songs.map((m) => m.toDomain()).toList(),
                                      startIndex: 0,
                                    ));
                                ...
                                if (widget.from == 'queue') {
                                  ...
                                  print(
                                    '🎵 Queue Play Next: currentIndex=$currentIndex, songIndex=$songIndex',
                                  );
                                  ...
                                  // final mutableSongs = List<SongsModel>.from(bloc.state.songs);
                                  ...
                                  try {
                                    ...
                                    print(
                                      '  Moving: $songIndex → $adjustedInsertIndex',
                                    );

                                    // Use the reorder method which handles both platforms correctly
                                    bloc.add(ReorderSongInQueueEvent(
                                      songIndex,
                                      adjustedInsertIndex,
                                    ));
                                    ...
                                    if (mounted) {
                                      final songToMove = bloc
                                          .state.songs[adjustedInsertIndex];
                                      showSnackBar(
                                        context,
                                        () {},
                                        message:
                                            '${songToMove.title} will play next',
                                        alertBannerLocation:
                                            AlertBannerLocation.bottom,
                                      );
                                    }
                                ...
                                /* if (widget.from == 'queue') {
                                    ...
                                    if (bloc.state.songs.isEmpty || widget.songIndex == null) {
                                      showSnackBar(
                                        context,
                                            () {},
                                        message: 'Unable to move song',
                                    ...
                                    final currentIndex = bloc.state.currentIndex ?? -1;
                                    final songIndex = widget.songIndex!;
                                ...
                                if (widget.from == 'playlist') {
                                  ...
                                  try {
                                    ...
                                    if (bloc.state.songs.isEmpty) {
                                      ...
                                      print(
                                        '📀 Inserting playlist after current song',
                                      );
                                      final currentIndex =
                                          bloc.state.currentIndex ?? -1;
                                      final newSongsList =
                                      ...
                                      print(
                                        '✅ Inserted ${songsToAdd.length} songs at position $insertIndex',
                                      );

                                      // ✅ CRITICAL FIX: Pass the current index to preserve playback
                                      bloc.add(
                                          UpdateSongsInQueueWithIndexEvent(
                                            newSongsList.map((m) => m.toDomain()).toList(),
                                            currentIndex,
                                          ));
                                    }
                                  ...
                                  if (bloc.state.songs.isEmpty) {
                                    print(
                                      '🎵 Play Next: Queue empty, starting song',
                                    );
                                    bloc.add(SetPlaylistEvent(
                                      [widget.currentSong!].map((m) => m.toDomain()).toList(),
                                      startIndex: 0,
                                      autoPlay: true,
                                    ));
                                  } else {
                                    ...
                                    print(
                                      '🎵 Play Next: Current index: $currentIndex, Insert at: $insertIndex',
                                    );
                                    ...
                                    log(
                                      '  Existing index in queue: $existingIndex',
                                    );
                                    if (existingIndex != -1 &&
                                        existingIndex != insertIndex) {
                                      print(
                                        '  Song exists at index $existingIndex, moving to $insertIndex',
                                      );
                                      ...
                                      print(
                                        '  Moved to adjusted index: $adjustedInsertIndex',
                                      );
                                    } else if (existingIndex == -1) {
                                      print(
                                        '  Song not in queue, inserting at $insertIndex',
                                      );
                                      bloc.add(PlayNextSingleSongEvent(
                                        widget.currentSong!.toDomain(),
                                      ));
                                      // newSongsList.insert(insertIndex, widget.currentSong!);
                                    } else {
                                      print(
                                        '  Song already at correct position',
                                      );
                                    }
                                    ...
                                    final newCurrentIndex =
                                        existingIndex != -1 &&
                                            existingIndex < insertIndex
                                        ? currentIndex -
                                              1 // If we moved a song from before current, current stays same
                                        : currentIndex; // Otherwise current index is unchanged

                                    print(
                                      '  New current index: $newCurrentIndex',
                                    );
                                  ...
                                  if (mounted) {
                                    showSnackBar(
                                      ...
                                      () {},
                                      message:
                                          '${widget.currentSong!.title} will play next',
                                      alertBannerLocation:
                                          AlertBannerLocation.bottom,
                                    );
                                  }
                                ...
                                /*if (widget.from == 'playlist') {
                                    ...
                                    try {
                                      ...
                                      if (bloc.state.songs.isEmpty) {
                                        ...
                                        print('📀 Inserting playlist after current song at index ${bloc.state.currentIndex ?? -1 + 1}');
                                        final currentIndex = bloc.state.currentIndex ?? -1;
                                        final newSongsList = List<SongsModel>.from(bloc.state.songs);
                                ...
                                if (widget.from == 'playlist') {
                                  ...
                                  showSnackBar(
                                    ...
                                    () {},
                                    message:
                                        "${newSongsList.length} songs added to queue",
                                    alertBannerLocation:
                                        AlertBannerLocation.bottom,
                                  );
                                } else {
                                  ...
                                  if (existingIndex == -1) {
                                    bloc.add(AddSingleSongToQueueEvent(
                                        widget.currentSong!.toDomain()));
                                    showSnackBar(
                                        context,
                                        () {},
                                ...
                                if (widget.from == 'queue') {
                                  ...
                                  print(
                                    '🗑️ Removing song from queue at index $removeIndex',
                                  );
                                  print('  Current index: $currentIndex');
                                  print(
                                    '  Queue size: ${bloc.state.songs.length}',
                                  );

                                  // ✅ CRITICAL: Capture playback state BEFORE any modifications
                                  final wasPlaying = bloc.state.isPlaying;
                                  final currentPosition = bloc.state.position;
                                  final isRemovingCurrent =
                                      removeIndex == currentIndex;

                                  // Create mutable copy
                                  final mutableSongs = List<SongsModel>.from(
                                    bloc.state.songs,
                                  );
                                  ...
                                  if (mutableSongs.isEmpty) {
                                    ...
                                    try {
                                      await bloc.player.stop();
                                      await bloc.player.seek(
                                        Duration.zero,
                                      );
                                    } catch (e) {
                                      print('⚠️ Error stopping player: $e');
                                    }
                                    bloc.add(const StopAndClearQueueEvent());
                                    if (context.mounted) {
                                      showSnackBar(
                                        context,
                                        () {},
                                  ...
                                  try {
                                    if (isRemovingCurrent) {
                                      ...
                                      final nextIndex =
                                          removeIndex < mutableSongs.length
                                          ? removeIndex
                                          : 0;
                                      print(
                                        '  Playing next at index: $nextIndex',
                                      );
                                      ...
                                      print(
                                        ...
                                        if (source
                                          ...
                                          print(
                                            '  Android: Using seamless removeAt()',
                                          );
                                      ...
                                      print('  Removing AFTER current');
                                        ...
                                        if (source
                                          ...
                                          print(
                                            '  Android: Rebuilding (shuffle enabled)',
                                          );
                                          bloc.add(
                                              UpdateSongsInQueueWithIndexEvent(
                                                mutableSongs.map((m) => m.toDomain()).toList(),
                                    ...
                                    if (wasPlaying && !bloc.state.isPlaying) {
                                      try {
                                        bloc.add(const PlayEvent());
                                      } catch (playError) {
                                        print(
                                          '❌ Failed to restore playback: $playError',
                                        );
                                      }
                                    }
                                  }
                                ...
                                if (!Platform.isAndroid) {
                                  showSnackBar(
                                    ...
                                    () {},
                                    message:
                                        'Ringtone feature is only available on Android',
                                    alertBannerLocation:
                                        AlertBannerLocation.bottom,
                                  );
                                  return;
                                ...
                                _hideSong(context.read<MusicPlayerBloc>());
                              }
                            },
                          ),
  ...
  /// Shown when deleting a song from folder context. Use when folder_in flow is enabled.
  // ignore: unused_element
  void _showDeleteFromFolderConfirmation(BuildContext context) {
    showCommonConfirmationBottomSheet(
      context: context,
      title: S.of(context).deleteSong,
      message: 'Are you sure you want to delete ${widget.currentSong?.title}?',
      onConfirm: (sheetContext) async {
        Navigator.pop(sheetContext);
        await _deleteSongFile(context);
      },
  ...
  Future<void> _deleteSongFile(BuildContext context) async {
    ...
    try {
      ...
      final currentSongs = List<SongsModel>.from(bloc.state.songs);
      currentSongs.removeWhere(
        (element) => element.id == widget.currentSong!.id,
      );

      if (currentSongs.isNotEmpty) {
        bloc.add(RemoveDeletedSongFromQueueEvent(widget.currentSong!.id!));
      } else {
  ...
  Future<void> _setAsRingtone(BuildContext context) async {
    ...
    try {
      ...
      try {
        ...
        final hasPermission =
            await platform.invokeMethod('checkWriteSettingsPermission')
                as bool? ??
            false;
          ...
          if (mounted) {
            final shouldOpenSettings = await showDialog<bool>(
              ...
              builder: (context) => AlertDialog(
                ...
                content: const Text(
                  'To set ringtone, please grant "Modify system settings" permission.',
                ),
                actions: [
                  TextButton(
          ...
          if (mounted) {
            if (setAsDefault) {
              ...
              showSnackBar(
                ...
                () {},
                message:
                    'Ringtone added. Please select "${song.title}" from Settings > Sound',
                alertBannerLocation: AlertBannerLocation.bottom,
              );
            } else {
              showSnackBar(
  ...
  void _hideSong(MusicPlayerBloc bloc) {
    showCommonConfirmationBottomSheet(
      context: context,
      title: 'Hide the Song',
      message: 'Are you sure you want to hide this song?',
      onConfirm: (sheetContext) async {
        try {
          context.read<SongsBloc>().add(
            SongsEvent.hideSong(widget.currentSong!.id!),
          );

          bloc.add(RemoveDeletedSongFromQueueEvent(
            widget.currentSong!.id!,
          ));
          ...
          if (mounted) {
            showSnackBar(
              context,
              () {},
              message: '"${widget.currentSong!.title}" hidden',
              alertBannerLocation: AlertBannerLocation.bottom,
            );
          }
