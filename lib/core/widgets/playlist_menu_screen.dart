...
class _PlaylistMenuScreenState extends State<PlaylistMenuScreen> {
  ...
  void _handlePlay() async {
    ...
    try {
      ...
      _bloc.add(SetPlaylistEvent(_songs.map((m) => m.toDomain()).toList(), startIndex: 0));
  ...
  void _handlePlayNext() async {
    ...
    try {
      ...
      if (_bloc.state.songs.isEmpty) {
        ...
        _bloc.add(PlayNextMultipleSongsEvent(_songs.map((m) => m.toDomain()).toList()));
        updateCount = _songs.length;
      }
      if (mounted) {
        if (updateCount < 1) {
  ...
  void _handleAddToQueue() async {
    ...
    try {
      ...
      _bloc.add(AddMultipleSongsToQueueEvent(_songs.map((m) => m.toDomain()).toList()));

      if (mounted) {
        showSnackBar(
          context,
          () {},
          message: "${_songs.length} songs added to queue",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
