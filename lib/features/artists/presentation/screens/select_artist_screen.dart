...
class _SelectArtistScreenState extends State<SelectArtistScreen> {
  ...
  void _playNextSelectedSongs() async {
    ...
    try {
      ...
      if (bloc.state.songs.isEmpty) {
        ...
        bloc.add(SetPlaylistEvent(allSongsFromArtists.map((m) => m.toDomain()).toList(), startIndex: 0));
        bloc.add(const PlayEvent());
      } else {
        bloc.add(PlayNextMultipleSongsEvent(allSongsFromArtists.map((m) => m.toDomain()).toList()));
        if (mounted) {
          showSnackBar(
            context,
            () {},
      ...
      // (comments about insert position removed)
    } catch (e) {
      if (mounted) {
        showSnackBar(
          context,
  ...
  void _addSelectedSongsToQueue() async {
    ...
    try {
      ...
      bloc.add(AddMultipleSongsToQueueEvent(allSongsFromArtists.map((m) => m.toDomain()).toList()));

      if (mounted) {
        showSnackBar(
          context,
          () {},
          message: "${allSongsFromArtists.length} songs from ${selectedArtists.length} artists added to queue",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
