...
class _GenreListScreenState extends State<GenreListScreen> {
  ...
  void _playGenre(Genre genre) async {
    try {
      ...
      bloc.add(SetPlaylistEvent(genreSongs.map((m) => m.toDomain()).toList(), startIndex: 0));
  ...
  void _playNextGenre(Genre genre) async {
    try {
      ...
      if (bloc.state.songs.isEmpty) {
        ...
        bloc.add(PlayNextMultipleSongsEvent(genreSongs.map((m) => m.toDomain()).toList()));
        showSnackBar(
          context,
          () {},
          message: "${genreSongs.length} songs added to play next",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
    } catch (e) {
  ...
  void _addGenreToQueue(Genre genre) async {
    try {
      ...
      final bloc = context.read<MusicPlayerBloc>();
      bloc.add(AddMultipleSongsToQueueEvent(genreSongs.map((m) => m.toDomain()).toList()));

      if (mounted) {
        showSnackBar(
          context,
          () {},
          message: "${genreSongs.length} songs added to queue",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
