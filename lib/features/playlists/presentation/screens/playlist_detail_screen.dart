...
class _PlaylistDetailScreenState extends State<PlaylistDetailScreen> {
  ...
  Future<void> _applyOrderAndKeepCurrent(List<SongsModel> newOrder) async {
    final currentId = _bloc.state.currentSongId;
    ...
    final shouldAutoplay = _bloc.state.isPlaying;
    _bloc.add(SetPlaylistEvent(
      newOrder.map((m) => m.toDomain()).toList(),
      startIndex: startIndex,
      autoPlay: shouldAutoplay,
    ));
    if (shouldAutoplay) _bloc.add(const PlayEvent());
  }
          ...
          MusicListTile(
            ...
            onTap: context.watch<HoldTheTapFor>().isHoldingSongPLay
                ...
                : () async {
                    ...
                    final current = _bloc.state.currentSong;
                    if (current != null &&
                        current.id == song.id &&
                        _bloc.state.isPlaying) {
                      context.push(
                        '/dashboard/playing',
                        extra: PlayingSongArgs(songs: _bloc.state.songs),
                      );
                    } else {
                      _bloc.add(SetPlaylistEvent(list.map((m) => m.toDomain()).toList(), startIndex: index));
                              ...
                              children: [
                                GestureDetector(
                                  onTap: context.watch<HoldTheTapFor>().isHoldingShuffle ? null : () async {
                                    ...
                                    if ((_bloc.state.currentIndex ?? -1) < 0) {
                                      log(
                                        'shuffle:- Current index is invalid, resetting to 0',
                                      );
                                      _bloc.add(SetPlaylistEvent(_songs.map((m) => m.toDomain()).toList(), autoPlay: false, startIndex: 0));
                                      _bloc.add(const PlayEvent());
                                    } else {
                                      log('shuffle:- Setting shuffle playlist');
                                      context.push('/dashboard/playing', extra: PlayingSongArgs(songs: _songs),);
                                      _bloc.add(SetShufflePlaylistEvent(_songs.map((m) => m.toDomain()).toList(), autoPlay: true));
                                      _bloc.add(const EnsureShuffleOnReshuffleIndexEvent());
                                      await _bloc.player.currentIndexStream.firstWhere((idx) => idx != null && idx != 0,);
                                      _bloc.add(const PlayEvent());
                                    }
                                ...
                                GestureDetector(
                                  onTap: context.watch<HoldTheTapFor>().isHoldingPlay ? null : () async {
                                    ...
                                    if(_bloc.state.isPlaying){
                                      context.push('/dashboard/playing', extra: PlayingSongArgs(songs: _songs),);
                                    }
                                    else if((_bloc.state.currentIndex ?? -1) >= 0){
                                      context.push('/dashboard/playing', extra: PlayingSongArgs(songs: _songs),);
                                      _bloc.add(const PlayEvent());
                                    }
                                    else{
                                      Future.microtask(() {
                                        _bloc.add(const EnsureShuffleOffEvent());
                                        _bloc.add(SetPlaylistEvent(List<SongsModel>.from(_baseSongs).map((m) => m.toDomain()).toList(), startIndex: 0, autoPlay: true));
                                        _bloc.add(const PlayEvent());
                                      });
                                    }
                                  },
