  ...
  PlaylistBloc(this.repository) : super(const PlaylistState.initial()) {
    on<_AddPlaylist>((event, emit) async {
      try {
        ...
        final songs = await repository.getSongsForSystemPlaylist('recently_played');
        final systemPlaylistSongs = <String, List<SongsModel>>{'recently_played': songs};
    ...
    on<_RenamePlaylist>((event, emit) async {
      try {
        final previousSystemSongs = state.maybeWhen(loaded: (_, systemPlaylistSongs) => systemPlaylistSongs, orElse: () => null);
        await repository.renamePlaylist(event.id, event.newName);
        final playlists = await repository.fetchAllPlaylists();
        emit(PlaylistState.loaded(playlists, systemPlaylistSongs: previousSystemSongs));
      } catch (e) {
        emit(PlaylistState.error(e.toString()));
      }
    });
    ...
    on<_AddMultipleSongsToPlaylist>((event, emit) async {
      try {
        log('Adding ${event.songIds.length} songs to playlist ${event.playlistId}');
        await repository.addMultipleSongsToPlaylist(event.playlistId, event.songIds);
        final playlists = await repository.fetchAllPlaylists();
    ...
    on<_RemoveMultipleSongsFromPlaylist>((event, emit) async {
      try {
        log('Removing ${event.songIds.length} songs from playlist ${event.playlistId}');
        await repository.removeMultipleSongsFromPlaylist(event.playlistId, event.songIds);
    ...
    on<_FetchSongsForSystemPlaylist>((event, emit) async {
      try {
        ...
        // Get songs for the specific system playlist
        final songs = await repository.getSongsForSystemPlaylist(event.systemKey);

        // Create a map with the system key and its songs
        final systemPlaylistSongs = <String, List<SongsModel>>{event.systemKey: songs};
    ...
    on<_GetFavoritesPlaylistId>((event, emit) async {
      try {
        final playlists = await repository.fetchAllPlaylists();
        final favoritesPlaylist = playlists.firstWhere((playlist) => playlist.systemKey == 'favorites', orElse: () => throw Exception('Favorites playlist not found'));
