...
class _SelectAlbumScreenState extends State<SelectAlbumScreen> {
  ...
  void _playNextSelectedSongs() async {
    ...
    try {
      ...
      if (bloc.state.songs.isEmpty) {
        ...
        bloc.add(SetPlaylistEvent(allSongsFromAlbums.map((m) => m.toDomain()).toList(), startIndex: 0));
        bloc.add(const PlayEvent());
      } else {
        bloc.add(PlayNextMultipleSongsEvent(allSongsFromAlbums.map((m) => m.toDomain()).toList()));
        if (mounted) {
          showSnackBar(
            context,
            () {},
            message: "${allSongsFromAlbums.length} songs from ${selectedAlbums.length} albums added to play next",
            alertBannerLocation: AlertBannerLocation.bottom,
          );
        }
      }
  ...
  void _addSelectedSongsToQueue() async {
    ...
    try {
      ...
      bloc.add(AddMultipleSongsToQueueEvent(allSongsFromAlbums.map((m) => m.toDomain()).toList()));

      if (mounted) {
        showSnackBar(
          context,
          () {},
          message: "${allSongsFromAlbums.length} songs from ${selectedAlbums.length} albums added to queue",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
