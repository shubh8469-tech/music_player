...
import 'package:music_app/features/playlists/presentation/screens/rename_playlist_bottom_sheet.dart';
                ...
                return MusicListTile(
                  ...
                  onTap: () async {
                    bloc.add(SetPlaylistEvent(
                      filtered.map((m) => m.toDomain()).toList(),
...
class _FolderActionSheet extends StatelessWidget {
  ...
  final Folder folder;
  FolderRepository get _repo => locator<FolderRepository>();
  UpdateFolderHiddenStatus get _updateFolderHiddenStatus =>
  ...
  Future<void> _playFolder(BuildContext context) async {
    ...
    try {
      ...
      bloc.add(SetPlaylistEvent(folderSongs.map((m) => m.toDomain()).toList(), startIndex: 0));
  ...
  Future<void> _playNextFolder(BuildContext context) async {
    ...
    try {
      ...
      if (bloc.state.songs.isEmpty) {
        ...
        bloc.add(PlayNextMultipleSongsEvent(folderSongs.map((m) => m.toDomain()).toList()));
        showSnackBar(
          context,
          () {},
          message: "${folderSongs.length} songs added to play next",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
    } catch (e) {
  ...
  Future<void> _addFolderToQueue(BuildContext context) async {
    ...
    try {
      ...
      final bloc = context.read<MusicPlayerBloc>();
      bloc.add(AddMultipleSongsToQueueEvent(folderSongs.map((m) => m.toDomain()).toList()));

      showSnackBar(
        context,
        () {},
        message: "${folderSongs.length} songs added to queue",
        alertBannerLocation: AlertBannerLocation.bottom,
      );
    } catch (e) {
      showSnackBar(
...
class _AlbumActionSheet extends StatelessWidget {
  ...
  final Album album;
  AlbumRepository get _repo => locator<AlbumRepository>();
...
class _ArtistActionSheet extends StatelessWidget {
  ...
  final Artist artist;
  ArtistRepository get _repo => locator<ArtistRepository>();
