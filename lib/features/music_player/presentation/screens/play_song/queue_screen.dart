...
class _QueueScreenState extends State<QueueScreen> {
  ...
  Future<void> _popIfQueueEmpty(List<SongsModel> queueSongs) async {
    ...
    _bloc.add(const StopAndClearQueueEvent());
  ...
  Future<void> _removeSongAtIndex(int index, List<SongsModel> queueSongs) async {
    ...
    final currentPlayingIndex = _bloc.state.currentIndex ?? -1;
    final isRemovingCurrentSong = index == currentPlayingIndex;

    final wasPlaying = _bloc.state.isPlaying;
    final currentPosition = _bloc.state.position;
    ...
    try {
      if (isRemovingCurrentSong) {
        ...
        await _bloc.player.stop();
        _bloc.add(SetPlaylistEvent(
          newQueue.map((m) => m.toDomain()).toList(),
        ...
        if (Platform.isAndroid) {
          final source = _bloc.player.audioSource;
          if (source is ConcatenatingAudioSource &&
              !_bloc.state.shuffleEnabled) {
            await source.removeAt(index);
            _bloc.add(UpdateSongsListEvent(newQueue.map((m) => m.toDomain()).toList()));
          } else {
            _bloc.add(UpdateSongsInQueueWithIndexEvent(
              newQueue.map((m) => m.toDomain()).toList(),
              newCurrentIndex,
            ));
            if (wasPlaying) {
              _bloc.add(SeekEvent(currentPosition, index: newCurrentIndex));
              _bloc.add(const PlayEvent());
            }
          }
        } else if (Platform.isIOS) {
          _bloc.add(RemoveFromQueueAtIndexEvent(index, newCurrentIndex));
          _bloc.add(UpdateSongsListEvent(newQueue.map((m) => m.toDomain()).toList()));
        }
      } else {
        if (Platform.isAndroid) {
      ...
      if (wasPlaying && !_bloc.state.isPlaying) {
        _bloc.add(const PlayEvent());
      }
    }
  }
  ...
  void _reorderSongs(int oldIndex, int newIndex, List<SongsModel> queueSongs) async {
    ...
    _isReordering = true;
    _bloc.add(PrepareReorderForCurrentSongEvent(oldIndex, newIndex));
    _updateDebounceTimer?.cancel();

    try {
      _bloc.add(SwapReorderSongInQueueEvent(oldIndex, newIndex));
    } finally {
      _isReordering = false;
    }
  }
  ...
  Future<void> _toggleShuffle(List<SongsModel> queueSongs) async {
    ...
    try {
      ...
      final enabled = _bloc.state.shuffleEnabled;
      showSnackBar(
        context,
        () {},
  ...
  Future<void> _toggleRepeat(List<SongsModel> queueSongs) async {
    ...
    try {
      ...
      final mode = _loopModeToString(_bloc.state.loopMode);
      String message;
      if (mode == 'off') {
        message = 'Repeat off';
      } else if (mode == 'one') {
  ...
  Future<void> _clearQueue() async {
    ...
    try {
      await _bloc.player.stop();
      await _bloc.player.seek(Duration.zero);
    } catch (e) {
      // Ignore
    }
    _bloc.add(const StopAndClearQueueEvent());
          ...
          songsState.maybeWhen(
            loaded: (librarySongs) {
              ...
              if (filteredQueue.length != queueSongs.length) {
                setState(() => selectedSongs.clear());
                if (_bloc.state.songs.isNotEmpty) {
                  _bloc.add(SetPlaylistEvent(
                    filteredQueue.map((m) => m.toDomain()).toList(),
                  ));
                }
                              ...
                              onTap: context
                                  ...
                                  : () async {
                                      ...
                                      if (state.songs.isNotEmpty &&
                                        ...
                                        _bloc.add(SetPlaylistEvent(
                                          queueSongs.map((m) => m.toDomain()).toList(),
                                          startIndex: index,
                                        ));
                                      }
                                    },
