...
class EqualizerPrefKeys {
  ...
  static const enabled = 'eq_enabled';
  static const preset = 'eq_preset';
  static const bandLevels = 'eq_band_levels';
}
...
class UnifiedEqualizerService {
  ...
  Future<void> initialize(dynamic player) async {
    ...
    // Ensure saved on/off state is applied (in case restore order or native layer changed it)
    final savedEnabled = _prefs.getBool(EqualizerPrefKeys.enabled) ?? false;
    if (_isEnabled != savedEnabled) {
      await setEnabled(savedEnabled);
    }
    log('═══════════════════════════════════════════════════');
  ...
  Future<void> restoreSavedEffects() async {
    ...
    // Restore on/off state first so it always applies even if other effect restores fail
    await setEnabled(savedEnabled);

    // Restore preset and band levels (Custom curve when preset was Custom)
    final savedPreset = _prefs.getString(EqualizerPrefKeys.preset) ?? 'Custom';
    final savedBandLevels = _prefs.getStringList(EqualizerPrefKeys.bandLevels);
    if (_presets.containsKey(savedPreset)) {
      if (savedPreset == 'Custom' && savedBandLevels != null && savedBandLevels.isNotEmpty) {
        final parsed = savedBandLevels
            .map((s) => double.tryParse(s) ?? 0.0)
            .toList();
        if (parsed.isNotEmpty) {
          if (parsed.length >= _bandLevels.length) {
            _bandLevels = List.from(parsed.take(_bandLevels.length));
          } else {
            _bandLevels = List.from(parsed)
              ..addAll(List.filled(_bandLevels.length - parsed.length, 0.0));
          }
  ...
  /// Persist the exact value from setBandLevel (UI scale) for this band.
  /// Restore uses this list and applies it when playing.
  void _saveBandLevelAt(int bandIndex, double gainUi) {
    try {
      List<String> list =
          _prefs.getStringList(EqualizerPrefKeys.bandLevels) ?? [];
      while (list.length <= bandIndex) list.add('0.0');
      list[bandIndex] = gainUi.toString();
      _prefs.setStringList(EqualizerPrefKeys.bandLevels, list);
    } catch (e) {
      log('❌ Failed to persist band level: $e');
    }
  }

  void _savePresetAndBandLevels() {
    try {
      _prefs.setString(EqualizerPrefKeys.preset, _currentPreset);
    } catch (e) {
      log('❌ Failed to persist preset: $e');
    }
  }
...
*/
  ...
  Future<void> setEnabled(bool enabled) async {
    ...
    try {
      await _prefs.setBool(EqualizerPrefKeys.enabled, enabled);
    } catch (e) {
      log('❌ Failed to persist equalizer enabled state: $e');
    }
  ...
  Future<void> setBandLevel(int bandIndex, double gain) async {
    ...
    _currentPreset = 'Custom';
    _presets['Custom'] = List.from(_bandLevels);
    _savePresetAndBandLevels();
    // Persist this band's value (UI scale) so we can restore it when playing
    _saveBandLevelAt(bandIndex, clampedGain);
 ...
 /* Future<void> _setAndroidReverb(String reverbType) async {
  ...
  Future<void> reset() async {
    ...
    await _prefs.remove(EqualizerPrefKeys.preset);
    await _prefs.remove(EqualizerPrefKeys.bandLevels);
  }
