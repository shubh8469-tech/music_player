...
class _SelectFolderScreenState extends State<SelectFolderScreen> {
  ...
  void _playNextSelectedSongs() async {
    ...
    try {
      ...
      if (bloc.state.songs.isEmpty) {
        ...
        bloc.add(SetPlaylistEvent(allSongsFromFolders.map((m) => m.toDomain()).toList(), startIndex: 0));
        bloc.add(const PlayEvent());
      } else {
        bloc.add(PlayNextMultipleSongsEvent(allSongsFromFolders.map((m) => m.toDomain()).toList()));
        if (mounted) {
          showSnackBar(
            context,
            () {},
  ...
  void _addSelectedSongsToQueue() async {
    ...
    try {
      ...
      bloc.add(AddMultipleSongsToQueueEvent(allSongsFromFolders.map((m) => m.toDomain()).toList()));

      if (mounted) {
        showSnackBar(
          context,
          () {},
          message: "${allSongsFromFolders.length} songs from ${selectedFolders.length} folders added to queue",
          alertBannerLocation: AlertBannerLocation.bottom,
        );
      }
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: Texts(
                              "${selectedItemsCount(filteredFolders, selectedFolderIds) != 0 ? "${selectedItemsCount(filteredFolders, selectedFolderIds)} ${S.of(context).selected}" : ''} ",
                              // selectedCount != 0
